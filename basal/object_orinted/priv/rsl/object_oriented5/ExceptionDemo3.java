...
class ExceptionDemo3 
{
	public static void main(String[] args)// throws Exception，也可以在这个地方抛出异常，但由于是jvm调用main，所以直接结束程序，所以一般不这样处理，使用try-catch功能
	{
		...
		catch (ArithmeticException e)/*注意，两个catch只能执行一个，因为先catch到的异常执行后，程序就结束了
										且catch要写针对性的处理，也可以直接写：catch (Exception e){System.out.println(e.toString());}
										但是此处理不具体，属于多态的catch处理方法，*/
		{
